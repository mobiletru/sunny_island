...
# Only files that ship in dashboards/ — no phantom webbox.yaml / plant.yaml.
LOVELACE_DASHBOARDS = {
    "sunny-island-pack": {
        "mode": "yaml",
...
def _dashboard_snippet() -> str:
    """YAML fragment the user can merge under lovelace.dashboards (no host rewrite)."""
    lines = [
...
def _install_dashboards(*, force: bool) -> list[str]:
    ...
    # Copy each shipped YAML into DST_DASH_DIR (no phantom root aliases).
    DST_DASH_DIR.mkdir(parents=True, exist_ok=True)
    for src in sorted(SRC_DASH.glob("*.yaml")):
        actions.append(_copy_file(src, DST_DASH_DIR / src.name, force=force))

    # Convenience root copy of pack detail (stable path for bookmarks).
...
def main() -> int:
    ...
    if not HA_CONFIG.is_dir():
        ...
        if SRC_EXAMPLES.is_dir():
            try:
                msg = _copy_tree(SRC_EXAMPLES, DST_EXAMPLES, force=force)
                actions.append(msg)
                print(f"[sunny_island] {msg}")
            except OSError as exc:
    ...
    # Optional mirror into image www/ for debugging (read-only images may fail).
    try:
